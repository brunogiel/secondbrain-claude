...
name: verificar
description: >
  Verifica un output contra lo que pediste y fact-checkea sus datos (números, fechas,
  citas, referencias, claims de "ya está deployado/mandado"). Sirve para el output de
  cualquier skill o de cualquier laburo. Usalo cuando digas "verificá esto", "chequeá
  que hiciste lo que pedí", "validá los datos / los números / las citas", "fact-check
  esto", "¿esto es real?", "revisá antes de mandar". Es el gate del ENTREGABLE (mandar
  o no mandar), distinto de evaluar-skill (que mejora el SKILL en el tiempo).
---
