...
Reglas: el coordinador queda fino (decide qué se reparte y cómo se junta); el laburo pesado vive en los skills/subagentes que llama. Paralelo solo cuando los pedazos son de verdad independientes; si cada paso necesita el anterior, es secuencial. Se aprende igual que un skill: usás uno de ejemplo (el de PPT) para ver el patrón y después armás el tuyo. El multi-agente brilla cuando algo que estás haciendo necesita varias miradas (ej. el `panel` del catálogo: 2-5 lentes en paralelo sobre el mismo artefacto, que te muestran ángulos y te devuelven la pelota).
...
Aparte queda **el método** (el coach con sus piezas, y en Claude Code el updater `actualizar`): vive como app fuera de tu brain, se llama por nombre y no aparece en tu `skills/`. Las **versiones-fuente** de los skills de uso (el catálogo) las lleva el coach adentro, en `kit/skills/`: copia de ahí a tu `skills/` cuando adoptás uno.
